...
export const setActiveFareRule = async (
  ...
  const result = await pool.query(
    ...
    [actor_user_id, line_code, base_price, base_stops, step_stops, step_price]
  );
  return unwrap(result);
};
...
export const getTopSpenders = async (from_date, to_date, limit, offset) => {
  const result = await pool.query(
    `SELECT api.fn_report_top_spenders_json($1, $2, $3, $4) AS result`,
    [from_date || null, to_date || null, limit, offset]
  );
  // Trả về { ok: true, total: ..., data: ... }
  return result.rows[0]?.result ?? { ok: true, total: 0, data: [] };
};
...
/**
 * 21) Lấy danh sách vé đã bán cho khách vãng lai (user_id IS NULL)
 */
export const getGuestTickets = async () => {
  const result = await pool.query(`
    SELECT 
      t.ticket_id,
      t.product_code,
      tp.name_vi as product_name,
      tp.type,
      t.from_station,
      t.to_station,
      t.stops,
    ...
    LEFT JOIN payments p ON t.ticket_id = p.ticket_id AND p.status = 'SUCCESS'
